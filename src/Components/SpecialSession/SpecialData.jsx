export default [
  {
    link: require("./sessions/SS-1.pdf"),
    title:
      "Media and Games in Education and Learning",
    organisers: [
      "Faizan Ahmad, Cardiff Metropolitan University, Cardiff UK.",
      "Momina Shaheen, University of Roehampton London, UK"
    ],
    listing: "SS 001",
  },
  {
    link: require("./sessions/SS-2.pdf"),
    title:
      "Advancements in Secure Communication Technologies: Unveiling the latest in wireless communication, IoT, and 5G networks with a dedicated security focus)",
    organisers: [
      "Dr. Parita Jain, KIET Group of Institutions, AKTU, India",
      "Dr. Puneet Kumar Aggarwal, ABES Engineering College, AKTU, India",
      "Dr. Sushil Kumar Singh, Marwadi University, Rajkot, Gujrat, India",
      "Mr. Mandeep Singh, Raj Kumar Goel Institute of Technology, AKTU, India"
    ],
    listing: "SS 002",
  },
  {
    link: require("./sessions/SS-3.pdf"),
    title:
      "Recent Advances in Computational Intelligence & Technologies",
    organisers: [
      "Dr. Vikash Yadav, Government Polytechnic Bighapur Unnao, Department of Technical Education, Uttar Pradesh, India",
      "Mr. Anup Lal Yadav, Chandigarh University, Mohali, Punjab, India",
      "Mr. Navjot Singh Talwandi, Chandigarh University, Mohali, Punjab, India"
    ],
    listing: "SS 003",
  },
  {
    link: require("./sessions/SS-4.pdf"),
    title:
      "Recent Advances in Software and System Engineering",
    organisers: [
      "Faizan Ahmad, Cardiff Metropolitan University, Cardiff, UK",
      "Zeeshan Ahmed, Institute of Software, University of Chinese Academy of Sciences, Beijing, China"
    ],
    listing: "SS 004",
  },
  {
    link: require("./sessions/SS-5.pdf"),
    title:
      "Recent Advancements in Generative AI: An Areas of Active Research",
    organisers: [
      "Dr. Narinder Kaur, Chandigarh University, Gharuan, Mohali, India",
      "Dr. Bobbinpreet Kaur, Chandigarh University, Gharuan, Mohali, India",
      "Dr. Puneet Kumar, Chandigarh University, Gharuan, Mohali, India",
      "Dr. Ganesh Gupta, Sharda University, Greater Noida, India",
      "Dr. Gaganjot Kaur,Raj Kumar Goel Institute of Technology, Ghaziabad,"
    ],
    listing: "SS 005",
  },
  {
    link: require("./sessions/SS-6.pdf"),
    title:
      "Advancements in Secure Communication Technologies: Unveiling the latest in wireless communication, IoT, and 5G networks with a dedicated security focus",
    organisers: [
      "Dr. Parita Jain, KIET Group of Institutions, AKTU, India",
      "Dr. Puneet Kumar Aggarwal, ABES Engineering College, AKTU, India",
      "Dr. Sushil Kumar Singh, Marwadi University, Rajkot, Gujrat, India",
      "Mr. Mandeep Singh, Raj Kumar Goel Institute of Technology, AKTU, India,"
    ],
    listing: "SS 006",
  },{
    link: require("./sessions/SS-7.pdf"),
    title:
      "Advancements in Communication and Networks using Data Analytics with Machine learning and Deep Learning",
    organisers: [
      "Dr. Preeti Nagrath, Associate Professor, Bharati Vidyapeeth’s college of Engineering, Delhi, India",
      "Dr. Rachna Jain, Associate Professor, Bhagwan Parshuram Institute of Technology, Delhi, India"
    ],
    listing: "SS 007",
  },
  {
    link: require("./sessions/SS-8.pdf"),
    title:
      "Computational intelligence in disease diagnosis, prediction and prevention",
    organisers: [
      "Dr. PRASANALAKSHMI B, College of Computer Science, King Khalid University, Abha, Saudi Arabia",
    ],
    listing: "SS 008",
  },
  {
    link: require("./sessions/SS-9.pdf"),
    title:
      "Computational Intelligence and Communication System",
    organisers: [
      "Dr. Tanweer Ali, Associate Professor, Department of Electronics and Communication Engineering, Manipal Institute of Technology, Manipal Academy of Higher Education,Manipal",
      "Dr. Praveen Kumar, Assistant Professor, Department of Electronics and Communication Engineering, Manipal Institute of Technology, Manipal Academy of Higher Education,Manipal",
      "Dr. Subramanya G Nayak, Professor, Department of Electronics and Communication Engineering, Manipal Institute of Technology, Manipal Academy of Higher Education, Manipal 576104, India",
      "Dr. Pramod Kumar, Professor, Department of Electronics and Communication Engineering, Manipal Institute of Technology, Manipal Academy of Higher Education, Manipal 576104, India",
      "Dr. Sameena Pathan, Assistant Professor, Department of Information and Communication Technology, Manipal Institute of Technology, Manipal Academy of Higher Education, Manipal 576104, India"
      
    ],
    listing: "SS 009",
  },
  {
    link: require("./sessions/SS-10.pdf"),
    title:
      "INTELLIGENT MACHINES AND COMPUTATIONAL SYSTEM ENGINEERING",
    organisers: [
      "Dr. Harish Kumar Taluja, HRIT Group of Institutions, Ghaziabad-201003,India",
      "Dr. Sonam Gupta, Ajay Kumar Garg Engineering College, Ghaziabad-201009, India", 
      "Dr.Anuradha Taluja, Ajay Kumar Garg Engineering College, Ghaziabad-201009, India"
      
    ],
    listing: "SS 010",
  },
  {
    link: require("./sessions/SS-11.pdf"),
    title:
      "Internet of Things (IOT) with Edge Computing on Healthcare, Agriculture, Education,Intelligent Transportation and Energy Management",
    organisers: [
      "Dr Biswa Mohan Sahoo, School of Engineering and Technology, Vivekananda Institute of Professional Studies-TC, Delhi",
      "Ms. Prerna Ajmani, School of Information Technology, Vivekananda Institute of Professional Studies-TC, Delhi"
      
    ],
    listing: "SS 011",
  },
  {
    link: require("./sessions/SS-12.pdf"),
    title:
      "Privacy, Security, and Integration Challenges in Smart Data Processing",
    organisers: [
      "Dr. Righa Tandon, Assistant Professor, Chitkara University, Rajpura, Punjab,India",
      "Dr. P.K Gupta, Professor, Jaypee University of Information Technology, Solan, H.P,India",
      "Dr. Md. Khubeb Siddiqui, Lead Data Scientist, IBM, Australia"      
    ],
    listing: "SS 012",
  },
  {
    link: require("./sessions/SS-13.pdf"),
    title:
      "Transforming the World to be Green for Humans with IoT and its Opportunities",
    organisers: [
      "Dr. A. Suresh, Associate Professor, Department of Networking and Communications,School of Computing, SRM Institute of Science and Technology, Chennai, Tamil Nadu, India",
      "Dr. V. Anbarasu, Associate Professor, Department of Networking and Communications, School of Computing, SRM Institute of Science and Technology, Chennai, Tamil Nadu, India"      
    ],
    listing: "SS 013",
  },
  {
    link: require("./sessions/SS-14.pdf"),
    title:
      "Computational Frontiers: Advancements in Computer Science and Technology using IoT & ML",
    organisers: [
      "Dr. Jay Kumar Pandey, Department of Electrical & Electronics Engineering , Shri Ramswaroop Memorial University , Dewa Road, Barabanki, U.P, India",
      "Dr. Mritunjay Rai, Department of Electrical & Electronics Engineering ,Shri Ramswaroop Memorial University , Dewa Road, Barabanki, U.P, India",
      "Mr. Ankur Gupta, Department of Computer Science and Engineering at Vaish College of Engineering, Rohtak"      
    ],
    listing: "SS 014",
  },
  {
    link: require("./sessions/SS-15.pdf"),
    title:
      "Cryptographic Techniques for Secure Machine Learning Inference",
    organisers: [
      "Dr. Manish Verma, Galgotias University, Gr. Noida, UP, India",
      "Dr. Ashutosh Dhar Dwivedi, Aalborg University, Copenhagen, Denmark",
      "Mr. Keshav Kaushik, University of Petroleum and Energy Studies, Dehradun,"      
    ],
    listing: "SS 015",
  },{
    link: require("./sessions/SS-16.pdf"),
    title:
      "AI in Healthcare",
    organisers: [
      "Dr.Balaji Ganesh Rajagopal, Assistant Professor, Department of Computer Science and Engineering, SRM Institute of Science and Technology, Tiruchirappalli, Tamil Nadu",
      "Dr.R.Deebalakshmi, Assistant Professor, Department of Computer Science and Engineering, SRM Institute of Science and Technology, Tiruchirappalli, TamilNadu"      
    ],
    listing: "SS 016",
  },{
    link: require("./sessions/SS-17.pdf"),
    title:
      "Recent Advances in AI and Machine Learning: Innovations, Environmental Sustainability, and Healthcare",
    organisers: [
      "Dr. Sangeeta Kumari, Bennett University (The Times Group), Greater Noida, U.P., India",
      "Dr. Sumit Kumar, Royal Holloway, University of London, UK"      
    ],
    listing: "SS 017",
  },{
    link: require("./sessions/SS-18.pdf"),
    title:
      "AI for sustainability: Exploring the Intersection of Sustainability and AI",
    organisers: [
      "Dr Shweta Bansal, K.R. Mangalam University, Sohna Road, Gurugram, Haryana, India",
      "Dr Nupur Jangu, K.R. Mangalam University, Sohna Road, Gurugram, Haryana, India",
      "Dr. Surabhi Shankar, K.R. Mangalam University, Sohna Road, Gurugram, Haryana, India"      
    ],
    listing: "SS 018",
  },
  {
    link: require("./sessions/SS-19.pdf"),
    title:
      "Intelligent System using Emerging Technologies for Smart Society",
    organisers: [
      "Prof.(Dr.) Kaushal Kisor, ABES Institute of Technology Ghaziabad, Uttar Pradesh India"      
    ],
    listing: "SS 019",
  },
  {
    link: require("./sessions/SS-20.pdf"),
    title:
      "AI-Driven Innovations in Networking: Enhancing Connectivity and Security",
    organisers: [
      "Dr. Neha Goel, Vivekananda Institute of Professional Studies-Technical Campus, Pitampura, Delhi, India",
      "Ms. Megha Bansal, Vivekananda Institute of Professional Studies-Technical Campus, Pitampura, Delhi, India",
      "Ms. Mitanshi Rustagi, Vivekananda Institute of Professional Studies-Technical Campus, Pitampura, Delhi, India",
      "Ms. Tanvi Jain, Vivekananda Institute of Professional Studies-Technical Campus, Pitampura, Delhi, India"      
    ],
    listing: "SS 020",
  }, 
  {
    link: require("./sessions/SS-21.pdf"),
    title:
      "Intelligent Systems in Action: Advances in AI, ML, and Expert Systems for Enhanced Security and Automation",
    organisers: [
      "Dr. Lokesh Jain, Department of Information Technology, Jagan Institute of Management Studies,Rohini - Delhi",
      "Dr. Vishwajeet Pattanaik, Specialist Scientist, Indian Institute of Science, Bangalore – India",
      "Dr. Shweta Suran, Visiting Research Fellow, Open University – United Kingdom,"      
    ],
    listing: "SS 021",
  }, 
  {
    link: require("./sessions/SS-22.pdf"),
    title:
      "AI in Blockchain towards Realizing Cyber Security",
    organisers: [
      "Mr. Alok Kumar, Galgotias College of Engineering and Technology, Greater Noida, India",
      "Dr. Rashmi Sharma, Ajay Kumar Garg Engineering College, Ghaziabad, India",
      "Dr. Inderjeet Kaur, Galgotias College of Engineering and Technology, Greater Noida, India"      
    ],
    listing: "SS 022",
  }, 
  {
    link: require("./sessions/SS-23.pdf"),
    title:
      "Leveraging Artificial Intelligence for Strategic Management: Innovations, Challenges, and Future Directions",
    organisers: [
      "Dr Reepu Chandigarh University India"      
    ],
    listing: "SS 023",
  }, 
  {
    link: require("./sessions/SS-24.pdf"),
    title:
      "Intelligent Information Processing Systems, Networks and Security",
    organisers: [
      "Dr. Rajdeep Chakraborty, Dept. of CSE, Medi-Caps University, Indore, Madhya Pradesh, India",
      "Dr. Partha Ghosh, Dept. of IT, Netaji Subhash Engineering College, Kolkata, West Bengal, India",
      "Dr. Ahmed A. Elngar, Faculty of Computers and Artificial Intelligence, Beni-Suef University,Beni-Suef City, Egypt"      
    ],
    listing: "SS 024",
  }, 
  {
    link: require("./sessions/SS-25.pdf"),
    title:
      "Integration of IoT and Big Data with Machine Learning and Blockchain",
    organisers: [
      "Dr. Neda Fatima, Associate Professor, Manav Rachna International Institute of Research and Studies"      
    ],
    listing: "SS 025",
  }, 
  {
    link: require("./sessions/SS-26.pdf"),
    title:
      "Applications of Artificial Intelligence and Machine Learning: Transforming Industries and Society",
    organisers: [
      "Dr Rashmi Agrawal, Professor, Manav Rachna International Institute of Research and Studies, Faridabad",
"Dr Parul Gandhi, Professor, Manav Rachna International Institute of Research and Studies, Faridabad,"      
    ],
    listing: "SS 026",
  }, 
  {
    link: require("./sessions/SS-27.pdf"),
    title:
      "AI-Enhanced Network Optimization: Innovations and Challenges",
    organisers: [
      "Dr. Deepti Khanna, Department of Information Technology, Jagan Institute of Management Studies, Rohini, Delhi, India"      
    ],
    listing: "SS 027",
  }, 
  {
    link: require("./sessions/SS-28.pdf"),
    title:
      "Recent Applications of Artificial Intelligence and Machine Learning",
    organisers: [
      "Dr. Ganesh Khekare, Vellore Institute of Technology, Vellore, India",
      " Dr. Gopal Sakarkar, Dr. Vishwanath Karad, MIT World Peace University, Pune, India"   
    ],
    listing: "SS 028",
  }, 
  {
    link: require("./sessions/SS-29.pdf"),
    title:
      "InnoNet: Innovation in Networking",
    organisers: [
      "Dr. Tanvi Gautam, Jaypee Institute of Information and Technology, India",
      "Dr. Prantik Biswas, Jaypee Institute of Information and Technology, India"   
    ],
    listing: "SS 029",
  }, 
  {
    link: require("./sessions/SS-30.pdf"),
    title:
      "Machine Learning in Network Optimization",
    organisers: [
      "Dr. Deepika Bansal, Maharaja Agrasen Institute of Technology, India",
      "Dr. Tejna Khosla, Maharaja Agrasen Institute of Technology, India"   
    ],
    listing: "SS 030",
  }, 
  {
    link: require("./sessions/SS-31.pdf"),
    title:
      "Role of AI and Machine Learning in Biomedical Engineering",
    organisers: [
      "Dr. Gurmeet Singh (PhD IITD, Associate Professor, GTBIT",
      "Dr. Ramandeep Singh, Scientist-D, Neuro-engineering Lab, Department of Neurosurgery, AIIMS, New Delhi"   
    ],
    listing: "SS 031",
  }, 

  
  
];
